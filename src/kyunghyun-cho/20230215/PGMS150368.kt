...
fun solution(users: Array<IntArray>, emoticons: IntArray): IntArray {
    ...
    combinationsWithRepetition(listOf(10, 20, 30, 40), emoticons.size).forEach { discountRates ->
        users.map { (minDiscountRateToBuy, minPriceToSubscribe) ->
            minPriceToSubscribe to
                discountRates.withIndex()
                    .filter { (_, discountRate) -> discountRate >= minDiscountRateToBuy }
                    .sumOf { (idx, discountRate) -> emoticons[idx] * (100 - discountRate) / 100 }
        }.partition { (minPriceToSubscribe, paid) -> paid >= minPriceToSubscribe }
